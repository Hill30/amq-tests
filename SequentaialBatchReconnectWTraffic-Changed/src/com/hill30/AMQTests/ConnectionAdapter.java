...
//*/
    ...
    private HashMap <Integer, Integer> messageMap = new HashMap();

    private int countSent = 0;
    public synchronized void publish(MqttAsyncClient client) {
        if (!canPublish) {
            return;
        }
        try {
    ...
    private synchronized void messageReceived(MqttMessage s) {
        try {
            ...
            if (this.clientID.equals(clientID)) {
                int val = messageMap.get(count);
                messageMap.put(count, ++val);
    ...
    public synchronized int[] checkForLostAndDups() {
        final int[] ret = {0, 0};
